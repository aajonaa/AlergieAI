...
# Run vLLM with 8-bit optimizations for 4090D
# Run vLLM with Native BF16 (Fastest Performance)
$PYTHON_EXEC -m vllm.entrypoints.openai.api_server \
    --model $MODEL_NAME \
    --trust-remote-code \
    --host 0.0.0.0 \
...
# Post-failure message (if OOM)
if [ $? -ne 0 ]; then
    echo "----------------------------------------------------------------"
    echo "ERROR: OOM or startup failed. Try these fixes:"
    echo "1. Use MODEL_NAME=\"TheBloke/Qwen3-7B-A3B-Instruct-2507-8bit\" (7B variant)"
    echo "2. Lower --gpu-memory-utilization to 0.85"
    echo "3. Reduce --max-model-len to 512"
    echo "----------------------------------------------------------------"
fi